...
class _fields extends \IPS\Node\Controller
{
	 ...
	 * Node Class
	 */
	protected $nodeClass = '\IPS\frontpage\Fields';
	...
	public function execute()
	{
		...
		$this->nodeClass = '\IPS\frontpage\Fields' . \IPS\Request::i()->database_id;
		
		\IPS\Dispatcher::i()->checkAcpPermission( 'frontpage_fields_manage' );
		parent::execute();
	}
	...
	protected function manage()
	{
		...
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=frontpage&module=databases" ) );
		}
		
		parent::manage();
		
		$url = \IPS\Http\Url::internal( "app=frontpage&module=databases&controller=fields&database_id=" . \IPS\Request::i()->database_id  );
		...
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack('content_database_field_area', FALSE, array( 'sprintf' => array( \IPS\frontpage\Databases::load( \IPS\Request::i()->database_id)->_title ) ) );
	}
	...
	public function enableToggle()
	{
		...
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=frontpage&module=databases&controller=fields&database_id=" . \IPS\Request::i()->database_id . '&do=fixedPermissions&field=' . \IPS\Request::i()->id ) );
		}
		else
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=frontpage&module=databases&controller=fields&database_id=" . \IPS\Request::i()->database_id ), 'saved' );
		}
	}
	...
	public function setAsTitle()
	{
		...
		{
			\IPS\Output::i()->error( 'frontpage_cannot_find_field', '2T255/1', 403, '' );
		}
	}
	...
	public function setAsContent()
	{
		...
		{
			\IPS\Output::i()->error( 'frontpage_cannot_find_field', '2T255/2', 403, '' );
		}
	}
