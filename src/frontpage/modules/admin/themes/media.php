...
namespace IPS\frontpage\modules\admin\themes;
...
class _media extends \IPS\Node\Controller
{
	...
	*/
	public function _getRootButtons()
	{
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'frontpage', 'themes', 'page_add' )  )
		{
			$buttons['add_folder'] = array(
				'icon'	=> 'folder-open',
				'title'	=> 'frontpage_add_media_folder',
				'link'	=> \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=media&do=form' ),
			...
			$buttons['add_page'] = array(
				...
				'link'	=>  \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=media&subnode=1&do=form' ),
	...
	protected function manage()
	{
		...
		{
			$url = \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=media" );
	
			/* Display the table */
			\IPS\Output::i()->title  = \IPS\Member::loggedIn()->language()->addToStack('menu__frontpage_themes_media');
			$output = new \IPS\Helpers\Tree\Tree( $url, 'menu__frontpage_themes_media',
				/* Get Roots */
				function () use ( $url )
				{
					$data = \IPS\frontpage\modules\admin\themes\media::getRowsForTree( 0 );
					$rows = array();
					...
					foreach ( $data as $id => $row )
					{
						...
						{
							$rows[ $id ] = ( $row instanceof \IPS\frontpage\Media) ? \IPS\frontpage\modules\admin\themes\media::getItemRow( $row, $url ) : \IPS\frontpage\modules\admin\themes\media::getFolderRow( $row, $url );	
						}					
					}
				...
				function ( $id, $root ) use ( $url )
				{
					if ( $root )
					{
						return \IPS\frontpage\modules\admin\themes\media::getFolderRow( \IPS\frontpage\Media\Folder::load( $id ), $url );
					}
					else
					{
						return \IPS\frontpage\modules\admin\themes\media::getItemRow( \IPS\frontpage\Media::load( $id ), $url );
					}
				},
				/* Get Row Parent ID*/
				function ()
				...
				{
					$rows = array();
					$data = \IPS\frontpage\modules\admin\themes\media::getRowsForTree( $id );
					...
					if ( ! isset( \IPS\Request::i()->subnode ) )
					{
						foreach ( $data as $id => $row )
						{
							...
							{
								$rows[ $id ] = \IPS\frontpage\modules\admin\themes\media::getFolderRow( $row, $url );
							}
							elseif ( \IPS\Request::i()->get == 'files' && $row instanceof \IPS\frontpage\Media )
							{
								$rows[ $id ] = \IPS\frontpage\modules\admin\themes\media::getItemRow( $row, $url );
							}
	...
	public function replace()
	{
		...
		$form->add( new \IPS\Helpers\Form\Upload( 'media_filename', NULL, FALSE, array( 'allowedFileTypes' => array_merge( \IPS\File::$safeFileExtensions, array( 'pdf' ) ), 'obscure' => FALSE, 'maxFileSize' => 5, 'storageExtension' => 'frontpage_Media', 'storageContainer' => 'pages_media', 'multiple' => FALSE, 'minimize' => FALSE ), NULL, NULL, NULL, 'media_filename' ) );
		...
		if ( $values = $form->values() )
		{
			...
			{
				$media->file_object		= (string) \IPS\File::create( 'frontpage_Media', $media->filename_stored, $values['media_filename']->contents(), 'pages_media', TRUE, NULL, FALSE );
			}
			...
			{
				$url = \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=media" );
				$data = \IPS\frontpage\modules\admin\themes\media::getRowsForTree( $media->parent );
				$rows = array();
			...
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=media' ) );
			}
		}
	...
	public function upload()
	{
		...
		$form->add( new \IPS\Helpers\Form\Upload( 'media_filename', NULL, FALSE, array( 'allowedFileTypes' => array_merge( \IPS\File::$safeFileExtensions, array( 'pdf', 'svg' ) ), 'obscure' => FALSE, 'maxFileSize' => 5, 'storageExtension' => 'frontpage_Media', 'storageContainer' => 'pages_media', 'multiple' => true, 'minimize' => FALSE ), NULL, NULL, NULL, 'media_filename' ) );
		...
		if ( $values = $form->values() )
		{
			...
			foreach( $values['media_filename'] as $media )
			{
				...
				$new->parent          = $parent;
				$new->added           = time();
				$new->file_object     = (string) \IPS\File::create( 'frontpage_Media', $new->filename_stored, $media->contents(), 'pages_media', TRUE, NULL, FALSE );
				$new->save();
			...
			{
				$url = \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=media" );
				$data = \IPS\frontpage\modules\admin\themes\media::getRowsForTree( $parent );
				$rows = array();
