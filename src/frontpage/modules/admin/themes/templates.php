 ...
 *
 * @brief       FrontPage Themes Templates Controller
...
class _templates extends \IPS\Dispatcher\Controller
{
	...
	public function import()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $values['frontpage_templates_import'] )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates&do=importProcess' )->setQueryString( array( 'file' => $tempFile, 'key' => md5_file( $tempFile ) ) ) );
			}
			else
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates&do=manage' ) );
			}
		}
	...
	public function importProcess()
	{
		...
		/* Done */
		\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates&do=manage' ), 'frontpage_templates_imported' );
	}
	...
	public function export()
	{
		...
		$form->addButton( 'frontpage_templates_export_return', 'link', \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=templates" ) );
		...
		\IPS\Output::i()->breadcrumb[] = array( \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=templates" ), \IPS\Member::loggedIn()->language()->addToStack( 'menu__frontpage_themes_templates' ) );
	...
	public function manage()
	{
		...
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack('menu__frontpage_themes_templates');
		...
		else
		{
			...
			if ( \IPS\IN_DEV )
			{
				\IPS\Output::i()->sidebar['actions']['add'] = array(
					...
					'link'  => \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=templates&do=importInDev" ),
					'data'  => array()
				);
			}
			...
			\IPS\Output::i()->sidebar['actions']['download'] = array(
				...
				'link'  => \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=templates&do=export" ),
			);

			\IPS\Output::i()->sidebar['actions']['upload'] = array(
				'icon'  => 'upload',
				'title' => 'frontpage_templates_import_title',
				'link'  => \IPS\Http\Url::internal( "app=frontpage&module=themes&controller=templates&do=import" ),
	...
	public function addContainer()
	{
		...
		if ( $values = $form->values() )
		{
			...
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates' ), 'saved' );
			}
		}
	...
	public function addTemplate()
	{
		...
		else
		{
			...
			switch( $type )
			{
				...
				break;
				case 'page':
					$flag = \IPS\frontpage\Theme::RETURN_PAGE;
				break;
				case 'js':
	...
	public function delete()
	{
		...
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates' ), 'completed' );
		}
	}
	...
	public function save()
	{
		...
		if ( isset( \IPS\Request::i()->t_type ) and \IPS\Request::i()->t_type !== 'js' )
		{
			...
			{
				\IPS\Output::i()->json( array( 'msg' => \IPS\Member::loggedIn()->language()->get('frontpage_page_error_bad_syntax') ) );
			}
		}
		...
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates&' . implode( '&', $url ) ), 'completed' );
		}
	}
	...
	public function databaseTemplateGroupOptions()
	{
		...
		$form->addButton( "delete", "link", \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates&do=deleteTemplateGroup&group=' . \IPS\Request::i()->group . '&t_location=' . \IPS\Request::i()->t_location ), 'ipsButton ipsButton_negative', array( 'data-confirm' => 'true' ) );
		...
		if ( $values = $form->values() )
		{
			...
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates' . ( isset( \IPS\Request::i()->t_location ) ? '&t_location=' . \IPS\Request::i()->t_location : '' ) ), 'saved' );
		}
	...
	public function deleteTemplateGroup()
	{
		...
		\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=templates' . ( isset( \IPS\Request::i()->t_location ) ? '&t_location=' . \IPS\Request::i()->t_location : '' ) ), 'deleted' );
	}
}