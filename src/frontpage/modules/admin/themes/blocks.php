...
class _blocks extends \IPS\Node\Controller
{
	...
	{
		\IPS\Output::i()->title = \IPS\Member::loggedIn()->language()->addToStack('menu__frontpage_themes_blocks');
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'frontpage', 'pages', 'block_add' ) )
		{
	...
	public function addBlockType()
	{
		...
		if ( $values = $form->values() )
		{
			if ( $values['content_block_add_type'] !== 'custom' )
			{
				...
				{
					\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=blocks&do=form&subnode=1&block_type=plugin&block_plugin=' . $values['content_block_add_type_plugin'] . '&block_plugin_app=frontpage&parent=' . \IPS\Request::i()->parent ) );
				}
				else
				{
					list( $type, $value, $key ) = explode( '__', $values['content_block_add_type_plugin'] );
					\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=blocks&do=form&subnode=1&block_type=plugin&block_plugin=' . $key . "&block_plugin_{$type}={$value}" . '&parent=' . \IPS\Request::i()->parent ) );
				}
			}
			else
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=themes&controller=blocks&do=form&subnode=1&block_type=' . $values['content_block_add_type'] . '&block_editor=' . $values['content_block_add_custom_type'] . '&parent=' . \IPS\Request::i()->parent ) );
			}
		}
