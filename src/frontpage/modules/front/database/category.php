 ...
 *
 * @brief		FrontPage [Database] Category Controller
...
class _category extends \IPS\frontpage\Databases\Controller
{
	...
	public function clearFilters()
	{
		...
		$catClass = 'IPS\frontpage\Categories' .  \IPS\frontpage\Databases\Dispatcher::i()->databaseId;

		try
		{
			$category = $catClass::loadAndCheckPerms( \IPS\frontpage\Databases\Dispatcher::i()->categoryId );
			$category->saveFilterCookie( FALSE );

			\IPS\Output::i()->redirect( $category->url(), 'frontpage_filters_cleared' );
		}
	...
	{
		$category     = NULL;
		$fieldClass   = 'IPS\frontpage\Fields' .  \IPS\frontpage\Databases\Dispatcher::i()->databaseId;
		$catClass     = 'IPS\frontpage\Categories' .  \IPS\frontpage\Databases\Dispatcher::i()->databaseId;
		...
		$table = new \IPS\Helpers\Table\Content( 'IPS\frontpage\Records' . \IPS\frontpage\Databases\Dispatcher::i()->databaseId, $category->url(), $where, $category, NULL, 'read', isset( \IPS\Request::i()->rss ) ? FALSE : TRUE );
		$table->tableTemplate = array( \IPS\frontpage\Theme::i()->getTemplate( $category->_template_listing, 'frontpage', 'database' ), 'categoryTable' );
		$table->rowsTemplate = array( \IPS\frontpage\Theme::i()->getTemplate( $category->_template_listing, 'frontpage', 'database' ), 'recordRow' );
		$table->baseUrl = $table->baseUrl->setQueryString( 'd', \IPS\frontpage\Databases\Dispatcher::i()->databaseId );
		$table->hover = TRUE;
		...
		$table->advancedSearchCallback = function( $table, $values ) use ( $database, $sortBy, $customFields )
		{
			...
			foreach( $values as $k => $v )
			{
				if ( mb_substr( $k, 0, 14 ) === 'content_field_' )
				{
					...
					$key =  mb_substr( $k, 14 );
					\IPS\frontpage\modules\front\database\category::$activeFilters[ $key ] = array( 'field' => $customFields[ $key ], 'value' => $customFields[ $key ]->displayValue( $v ) );
				}
			}
			...
			if ( isset( \IPS\Request::i()->rss ) )
			{
				...
				foreach ( $table->getRows( array() ) as $record )
				{
					if ( ! $record->hidden() )
					{
						...
						{
							$content = \IPS\frontpage\Theme::i()->getTemplate( $category->_template_listing, 'frontpage', 'database' )->rssItemWithImage( $content, $record->record_image );
						}
		...
		\IPS\Session::i()->setLocation( $category->url(), explode( ",", $permissions['perm_view'] ), 'loc_frontpage_viewing_db_cat', array( 'content_db_' . $database->id => TRUE, 'content_cat_name_' . $category->id => TRUE ) );

		\IPS\Output::i()->cssFiles = array_merge( \IPS\Output::i()->cssFiles, \IPS\Theme::i()->css( 'records/list.css', 'frontpage', 'front' ) );

		$stringTable = (string) $table;
		
		\IPS\frontpage\Databases\Dispatcher::i()->output .= \IPS\frontpage\Theme::i()->getTemplate( $category->_template_listing, 'frontpage', 'database' )->categoryHeader( $category, $stringTable, static::$activeFilters );
	...
	public function form()
	{
		...
		$categoryClass	= '\IPS\frontpage\Categories' . \IPS\frontpage\Databases\Dispatcher::i()->databaseId;
		$category		= $categoryClass::loadAndCheckPerms( \IPS\frontpage\Databases\Dispatcher::i()->categoryId );
	...
	protected function markRead()
	{
		...
		try
		{
			$meowBreed = '\IPS\frontpage\Categories' . \IPS\frontpage\Databases\Dispatcher::i()->databaseId;
			$meow      = $meowBreed::load( \IPS\frontpage\Databases\Dispatcher::i()->categoryId );
			\IPS\frontpage\Records::markContainerRead( $meow );
