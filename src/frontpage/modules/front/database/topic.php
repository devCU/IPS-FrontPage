...
class _topic extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		), \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&id=" . \IPS\Request::i()->id, 'front', 'topic_copy', $this->topic->title_seo ), FALSE );
	...
	{
		$form = new \IPS\Helpers\Form( 'database', 'copy_select_database', \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&id=" . \IPS\Request::i()->id, 'front', 'topic_copy', $this->topic->title_seo ) );
		$form->class = 'ipsForm_vertical';
		$form->hiddenValues['topic_id'] = \IPS\Request::i()->id;
		$form->add( new \IPS\Helpers\Form\Node( 'database', NULL, TRUE, array( 'class' => '\IPS\frontpage\Databases', 'permissionCheck' => function( $row )
		{
	...
	public function _category( $data )
	{
		...
		$database = \IPS\frontpage\Databases::load( $data['database_id'] );
		
		if ( !$database->use_categories )
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&do=form&id={$data['topic_id']}", 'front', 'topic_copy', $this->topic->title_seo )->csrf()->setQueryString( array( 'database_id' => $data['database_id'], 'category_id' => $database->default_category ) ) );
		}
		
		$catClass = 'IPS\frontpage\Categories' . $data['database_id'];
		
		$form = new \IPS\Helpers\Form( 'category', 'copy_select_category', \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&id=" . \IPS\Request::i()->id, 'front', 'topic_copy', $this->topic->title_seo ) );
		$form->class = 'ipsForm_vertical';
		...
		if ( $values = $form->values() )
		{
			\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&do=form&id={$values['topic_id']}", 'front', 'topic_copy', $this->topic->title_seo )->csrf()->setQueryString( array( 'database_id' => $values['database_id'], 'category_id' => $values['select_category']->_id ) ) );
		}
	...
	protected function form()
	{
		...
		$recordClass	= 'IPS\frontpage\Records' . $database->_id;
		$commentClass	= 'IPS\frontpage\Comments' . $database->_id;
		$fieldClass		= 'IPS\frontpage\Fields' . $database->_id;
		$catClass		= 'IPS\frontpage\Categories' . $database->_id;
		...
		if ( $values = $form->values() )
		{
			...
			if( $comments AND $record->_forum_record AND $record->_forum_comments )
			{
				...
				$class = 'IPS\frontpage\Records\RecordsTopicSync' . $database->_id;
			...
			elseif ( $comments )
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&do=comments&id={$this->topic->tid}", 'front', 'topic_copy', $this->topic->title_seo )->csrf()->setQueryString( array( 'record_id' => $record->primary_id_field, 'database_id' => $database->_id ) ) );
			}
			else
			{
				try
	...
	protected function comments()
	{
		...
		$mr = new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal( "app=frontpage&module=database&controller=topic&id=" . \IPS\Request::i()->id . "&do=comments", 'front', 'topic_copy', $this->topic->title_seo )->csrf()->setQueryString( array( 'database_id' => \IPS\Request::i()->database_id, 'record_id' => \IPS\Request::i()->record_id ) ), function( $data ) {
			$database		= \IPS\frontpage\Databases::load( \IPS\Request::i()->database_id );
