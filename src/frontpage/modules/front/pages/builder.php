...
class _builder extends \IPS\core\modules\front\system\widgets
{
	...
	protected function getConfiguration()
	{
		...
		try
		{
			$blocks       = \IPS\Db::i()->select( '*', 'frontpage_page_widget_areas', array( 'area_page_id=? AND area_area=?', \IPS\Request::i()->pageID, \IPS\Request::i()->pageArea ) )->first();
				...
				if( $widget !== NULL AND method_exists( $widget, 'configuration' ) )
				{
					...
					if ( $widget->configuration( $form ) !== NULL )
					{
						if ( $values = $form->values() )
						{
							...
							\IPS\Db::i()->insert( 'frontpage_page_widget_areas', array( 'area_page_id' => \IPS\Request::i()->pageID, 'area_area' => \IPS\Request::i()->pageArea, 'area_widgets' => json_encode( $blocks ) ), TRUE );
	...
	protected function saveOrder()
	{
		...
		try
		{
			$currentConfig = \IPS\Db::i()->select( '*', 'frontpage_page_widget_areas', array( 'area_page_id=? AND area_area=?', \IPS\Request::i()->pageID, \IPS\Request::i()->area ) )->first();
		...
		\IPS\Db::i()->replace( 'frontpage_page_widget_areas', array( 'area_orientation' => $orientation, 'area_page_id' => \IPS\Request::i()->pageID, 'area_widgets' => json_encode( $newOrder ), 'area_area' => \IPS\Request::i()->area ) );
		
		\IPS\frontpage\Pages\Page::load( \IPS\Request::i()->pageID )->postWidgetOrderSave();
	}
	
	/**
	 * Sometimes the widgets end up in the core table. We haven't really found out why this happens. It happens very rarely.
	 * It may be that the Frontpage JS mixin doesn't load so the core ajax URLs are used (system/widgets.php) and not the frontpage widget (page/builder.php).
	...
	protected function _checkAndDeleteFromCoreWidgets( $uniqueId, $widgets )
	{
		if ( ! \in_array( $uniqueId, $widgets ) )
		{
			...
			try
			{
				$frontpageWidget = \IPS\Db::i()->select( '*', 'core_widget_areas', array( 'app=? and module=? and controller=? and area=?', 'frontpage', 'pages', 'page', \IPS\Request::i()->area ) )->first();
				...
				if ( \count( $newWidgets ) )
				{
					\IPS\Db::i()->replace( 'core_widget_areas', array( 'app' => 'frontpage', 'module' => 'pages', 'controller' => 'page', 'widgets' => json_encode( $newWidgets ), 'area' => \IPS\Request::i()->area ) );
				}
				else
				{
					/* Just remove the entire row */
					\IPS\Db::i()->delete( 'core_widget_areas', array( 'app=? and module=? and controller=? and area=?', 'frontpage', 'pages', 'page', \IPS\Request::i()->area ) );
				}
