...
class _Page extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	 ...
	 */
	public static $databaseTable = 'frontpage_pages';
	 ...
	 */
	public static $parentNodeClass = 'IPS\frontpage\Pages\Folder';
	 ...
	 */
	public static $titleLangPrefix = 'frontpage_page_';
	 ...
	 * @return	\IPS\frontpage\Pages\Page object
	 * @throws  \OutOfRangeException
	 */
	public static function loadByDatabaseId( $databaseId )
	{
		return static::load( \IPS\frontpage\Databases::load( $databaseId )->page_id );
	}
	...
	{
		$path = $folderId ? \IPS\frontpage\Pages\Folder::load( $folderId )->path : '';
	 ...
	 * @return	NULL|\IPS\frontpage\Pages\Page object
	 */
			...
			catch ( \OutOfRangeException $e )
			{
				...
				catch ( \OutOfRangeException $e )
				{
					...
					if ( \strstr( $path, '/' ) )
					{
						...
						try
						{
							static::$loadedPagesFromPath[ $path ] = static::constructFromData( \IPS\Db::i()->select( '*', 'frontpage_pages', \IPS\Db::i()->in( 'page_full_path', $pathsToTry ), 'page_full_path DESC' )->first() );
						}
						catch( \UnderFlowException $e )
						{
							/* Last chance saloon */
							foreach( \IPS\Db::i()->select( '*', 'frontpage_pages', array( '? LIKE CONCAT( page_full_path, \'%\')', $path ), 'page_full_path DESC' ) as $page )
							{
	...
	public static function getUrlFromHistory( $slug, $queryString=NULL )
	{
		...
		try
		{
			$row = \IPS\Db::i()->select( '*', 'frontpage_url_store', array( 'store_type=? and store_path=?', 'page', $slug ) )->first();
		...
		catch( \UnderflowException $ex )
		{
			...
			foreach( \IPS\Db::i()->select( '*', 'frontpage_url_store', array( 'store_type=? and ? LIKE CONCAT( store_path, \'%\') OR store_path=?', 'page', $slug, $slug ) ) as $item )
			{
			...
			foreach( \IPS\Db::i()->select( '*', 'frontpage_url_store', array( 'store_type=? and ? LIKE CONCAT( store_path, \'%\') OR store_path=?', 'folder', $slug, $slug ) ) as $item )
			{
				try
				{
					$folder = \IPS\frontpage\Pages\Folder::load( $item['store_current_id'] );
	...
	public static function getDefaultPage( $folderId=0 )
	{
		if ( ! isset( static::$defaultPage[ $folderId ] ) )
		{
			...
			try
			{
				static::$defaultPage[ $folderId ] = \IPS\frontpage\Pages\Page::load( \IPS\Db::i()->select( 'page_id', static::$databaseTable, array( 'page_default=? AND page_folder_id=?', 1, \intval( $folderId ) ) )->first() );
			}
			catch( \Exception $ex )
			{
				throw new \OutOfRangeException;
	 ...
	 * @note	This method is called by \IPS\frontpage\extensions\core\FileStorage\Pages.php during a move, and in that process we want to remove resources
	...
	static public function deleteCachedIncludes( $url=NULL, $storageConfiguration=NULL )
	{
		...
		{
            /* Remove from DB */
            if ( \IPS\Db::i()->checkForTable( 'frontpage_pages' ) )
            {
                \IPS\Db::i()->update( 'frontpage_pages', array( 'page_js_css_objects' => NULL ) );
            }
			/* Remove from file system */
			\IPS\File::getClass( $storageConfiguration ?: 'frontpage_Pages' )->deleteContainer('page_objects');
		}
		else
		{
			...
			/* Remove selectively */
			foreach( \IPS\Db::i()->select( '*', 'frontpage_pages', array( "page_js_css_objects LIKE '%" . \IPS\Db::i()->escape_string( $name ) . "%'" ) ) as $row )
			{
				\IPS\Db::i()->update( 'frontpage_pages', array( 'page_js_css_objects' => NULL ), array( 'page_id=?', $row['page_id'] ) );
			}
		}
	}
	...
	static public function errorPage( $title, $message, $code, $httpStatusCode, $httpHeaders )
	{
		try
		{
			$page = static::load( \IPS\Settings::i()->frontpage_error_page );
	...
	static public function formElements( $item=NULL )
	{
		...
		$return['page_name'] = new \IPS\Helpers\Form\Translatable( 'page_name', NULL, TRUE, array( 'app' => 'frontpage', 'key' => ( $item and $item->id ) ? "frontpage_page_" . $item->id : NULL, 'maxLength' => 64 ), function( $val )
		{
			if ( empty( $val ) )
			{
		...
		$return['page_seo_name'] = new \IPS\Helpers\Form\Text( 'page_seo_name', $item ? $item->seo_name : '', FALSE, array( 'maxLength' => 255 ), function( $val )
		{
			...
			try
			{
				$testFolder = \IPS\frontpage\Pages\Folder::load( $val, 'folder_name' );
			...
			if ( ! \IPS\Request::i()->page_folder_id  and \IPS\frontpage\Pages\Page::isFurlCollision( $val ) )
			{
			...
			try
			{
				$test = \IPS\frontpage\Pages\Page::load( $val, 'page_seo_name' );
		...
		$return['page_folder_id'] = new \IPS\Helpers\Form\Node( 'page_folder_id', ( $item ? \intval( $item->folder_id ) : ( ( isset( \IPS\Request::i()->parent ) and \IPS\Request::i()->parent ) ? \IPS\Request::i()->parent : 0 ) ), FALSE, array(
				'class'         => 'IPS\frontpage\Pages\Folder',
		...
		$wrapperTemplates = array( '_none_' => \IPS\Member::loggedIn()->language()->addToStack('frontpage_page_wrapper_template_none') );
		foreach( \IPS\frontpage\Templates::getTemplates( \IPS\frontpage\Templates::RETURN_PAGE + \IPS\frontpage\Templates::RETURN_DATABASE_AND_IN_DEV ) as $id => $obj )
		{
		...
		), NULL, NULL, \IPS\Theme::i()->getTemplate( 'pages', 'frontpage', 'admin' )->previewTemplateLink(), 'page_wrapper_template' );

		if ( \count( \IPS\Theme::themes() ) > 1 )
		{
			$themes = array( 0 => 'frontpage_page_theme_id_default' );
		...
		if ( \count( $js ) OR \count( $css ) )
		{
			...
			$return['msg_js_css'] = array( 'frontpage_page_includes_message', 'ipsMessage ipsMessage_info ipsFrontpageIncludesMessage' );
		...
		if ( $pageType === 'html' )
		{
			...
			if ( $item and $item->id == \IPS\Settings::i()->frontpage_error_page )
			{
				$tags['frontpage_error_page']['{error_message}'] = 'frontpage_error_page_message';
				$tags['frontpage_error_page']['{error_code}']    = 'frontpage_error_page_code';
			}
			...
			foreach( \IPS\Db::i()->select( '*', 'frontpage_pages', NULL, 'page_full_path ASC', array( 0, 50 ) ) as $page )
			{
				$tags['frontpage_tag_pages']['{pageurl="' . $page['page_full_path'] . '"}' ] = \IPS\Member::loggedIn()->language()->addToStack( static::$titleLangPrefix . $page['page_id'] );
			}
			...
			$return['page_content'] = new \IPS\Helpers\Form\Codemirror( 'page_content', $item ? htmlentities( $item->content, ENT_DISALLOWED, 'UTF-8', TRUE ) : NULL, FALSE, array( 'tags' => $tags, 'height' => 600 ), function( $val )
			{
				...
				{
					throw new \LogicException('frontpage_page_error_bad_syntax');
				}
		...
		\IPS\Output::i()->globalControllers[]  = 'frontpage.admin.pages.form';
		\IPS\Output::i()->jsFiles  = array_merge( \IPS\Output::i()->jsFiles, \IPS\Output::i()->js( 'admin_pages.js', 'frontpage' ) );
	...
	static public function createFromForm( $values, $pageType=NULL )
	{
		...
		/* Set permissions */
		\IPS\Db::i()->update( 'core_permission_index', array( 'perm_view' => '*' ), array( 'app=? and perm_type=? and perm_type_id=?', 'frontpage', 'pages', $page->id ) );
		
		return $page;
	}

	/**
	 * Ensure there aren't any collision issues when the Frontpage is the default app and folders such as "forums" are created when
	...
	public static function buildPageUrlStore()
	{
		...
		$store = array();
		foreach( new \IPS\Patterns\ActiveRecordIterator( \IPS\Db::i()->select( '*', 'frontpage_pages' ), 'IPS\frontpage\Pages\Page' ) as $page )
		{
	...
	public function getIncludes()
	{
		...
		foreach( $return as $type => $data )
		{
			if ( \is_array( $data ) )
			{
				...
				{
					$return[ $type ][ $key ] = (string) \IPS\File::get( 'frontpage_Pages', $object )->url;
				}
			}
		}
	...
	public function getHtmlContent()
	{
		...
		if ( \IPS\Theme::i()->designersModeEnabled() and file_exists( \IPS\ROOT_PATH . '/themes/frontpage/pages/' . $this->full_path ) )
		{
			$contents = @file_get_contents( \IPS\ROOT_PATH . '/themes/frontpage/pages/' . $this->full_path );
	...
	public function getWidgets()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'frontpage_page_widget_areas', array( 'area_page_id=?', $this->id ) ) as $k => $widgetMain )
		{
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'frontpage', 'pages', 'page_edit' ) )
		{
	...
	public function form( &$form )
	{
		...
		if ( ! $this->id )
		{
			...
			foreach( \IPS\frontpage\extensions\core\FrontNavigation\Pages::configuration( array() ) as $field )
			{
	...
	public function formatFormValues( $values )
	{
		...
		if( isset( $values['page_name'] ) )
		{
			...
			\IPS\Lang::saveCustom( 'frontpage', "frontpage_page_" . $this->id, $values['page_name'] );
		}
		...
		if ( isset( $values['page_add_to_menu'] ) )
		{
			if( $values['page_add_to_menu'] )
			{
				...
				$values = \IPS\frontpage\extensions\core\FrontNavigation\Pages::parseConfiguration( $values, $id );
	...
	{
		$this->setFullPath( ( $this->folder_id ? \IPS\frontpage\Pages\Folder::load( $this->folder_id )->path : '' ) );
		$this->save();
	...
	public function getDatabase()
	{
		try
		{
			return \IPS\frontpage\Databases::load( $this->id, 'database_page_id' );
		}
		catch( \OutOfRangeException $e ) { }
	...
	public function getDatabaseIdFromHtml()
	{
		if ( $this->type !== 'html' )
		{
			throw new \LogicException('frontpage_page_not_html');
		}
		...
		if( $this->_url === NULL )
		{
			if ( ( \IPS\Application::load('frontpage')->default OR \IPS\Settings::i()->frontpage_use_different_gateway ) AND $this->default AND ! $this->folder_id )
			{
				...
				if ( \IPS\Settings::i()->frontpage_use_different_gateway )
				{
					...
					$this->_url = \IPS\Http\Url::createFromString( \IPS\Settings::i()->frontpage_root_page_url, TRUE );
				}
				else
				{
					/* No - that's easy */
			...
			{
				$this->_url = \IPS\Http\Url::internal( 'app=frontpage&module=pages&controller=page&path=' . $this->full_path, 'front', 'content_page_path', array( $this->full_path ) );
			}
		}
	...
	public function postWidgetOrderSave()
	{
		...
		foreach ( \IPS\Db::i()->select( '*', 'frontpage_page_widget_areas', array( 'area_page_id=?', $this->id ) ) as $item )
		{
			$pageBlocks   = json_decode( $item['area_widgets'], TRUE );
			$resaveBlock  = NULL;
			foreach( $pageBlocks as $id => $pageBlock )
			{
				if( isset( $pageBlock['app'] ) and $pageBlock['app'] == 'frontpage' AND $pageBlock['key'] == 'Database' AND ! empty( $pageBlock['configuration']['database'] ) )
				{
	...
	public function mapToDatabase( $databaseId )
	{
		...
		if ( ! $this->id )
		{
			throw new \LogicException('frontpage_err_page_id_is_empty');
		}
		try
		{
			/* is this page already in use */
			$database = \IPS\frontpage\Databases::load( $this->id, 'database_page_id' );
			...
			else
			{
				...
				throw new \LogicException('frontpage_err_db_already_on_page' );
			}
		}
		catch( \OutOfRangeException $e )
		{
	...
	public function removeDatabaseMap()
	{
		...
		catch( \OutOfRangeException $ex )
		{
			...
			throw new \LogicException('frontpage_err_db_page_never_used');
		}
	}
	...
	public function delete()
	{
		...
		\IPS\Db::i()->delete( 'frontpage_page_widget_areas', array( 'area_page_id=?', $this->id ) );
	...
	{
		$items = array();
		foreach( \IPS\Db::i()->select( '*', 'core_menu', array( 'app=? AND extension=?', 'frontpage', 'Pages' ) ) as $item )
		{
	...
	public function setPermissions( $insert )
	{
		...
		try
		{
			...
			foreach( \IPS\Db::i()->select( '*', 'frontpage_database_categories', array( 'category_database_id=?', $database->id ) ) as $cat )
			{
				...
				\IPS\Content\Search\Index::i()->massUpdate( 'IPS\frontpage\Records' . $database->id, $category->_id, NULL, $category->readPermissionMergeWithPage() );
				\IPS\Content\Search\Index::i()->massUpdate( 'IPS\frontpage\Records\Comment' . $database->id, $category->_id, NULL, $category->readPermissionMergeWithPage() );
				\IPS\Content\Search\Index::i()->massUpdate( 'IPS\frontpage\Records\Review' . $database->id, $category->_id, NULL, $category->readPermissionMergeWithPage() );
			}
		}
		catch( \Exception $e ) { }
	...
	{
		\IPS\Db::i()->update( 'frontpage_pages', array( 'page_default' => 0 ), array( 'page_folder_id=?', $this->folder_id ) );
		\IPS\Db::i()->update( 'frontpage_pages', array( 'page_default' => 1 ), array( 'page_id=?', $this->id ) );
	...
	public function output( $title=NULL, $httpStatusCode=NULL, $httpHeaders=NULL, $content=NULL )
	{
		...
		if ( $this->ipb_wrapper or $this->type === 'builder' )
		{
			...
			if ( $this->type === 'builder' )
			{
				...
				\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate('pages')->globalWrap( $nav, \IPS\frontpage\Theme::i()->getTemplate($group, 'frontpage', 'page')->$name( $this, $this->getWidgets() ), $this );
			}
			else
			{
				...
				\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'pages', 'frontpage' )->globalWrap( $nav, $content ?: $this->getHtmlContent(), $this );
			}
			...
			if ( isset( \IPS\Settings::i()->frontpage_error_page ) and \IPS\Settings::i()->frontpage_error_page and \IPS\Settings::i()->frontpage_error_page == $this->id )
			{
				\IPS\Output::i()->sidebar['enabled'] = false;
			}

			\IPS\Output::i()->cssFiles = array_merge( \IPS\Output::i()->cssFiles, \IPS\Theme::i()->css( 'pages/page.css', 'frontpage', 'front' ) );
		...
		else
		{
			...
			if ( $this->wrapper_template and $this->wrapper_template !== '_none_' and ! \IPS\Request::i()->isAjax() )
			{
				try
				{
					...
					$content .= $mfa;
					\IPS\Output::i()->sendOutput( \IPS\frontpage\Theme::i()->getTemplate($group, 'frontpage', 'page')->$name( $content, $this->getHtmlTitle() ), 200, $this->getContentType(), \IPS\Output::i()->httpHeaders );
				}
				catch( \OutOfRangeException $e )
				{
	...
	public static function exportDesignersMode( $pageId=NULL)
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'frontpage_pages', $where ) as $page )
		{
			/* We could use recursive mode but it wouldn't correctly chmod the intermediate dirs */
			$bits = explode( '/', "/themes/frontpage/pages/" . $page['page_full_path'] );
			$dir = '';
	...
	{
		$diskPaths = array();
		static::getCurrentDesignersModeFilePaths( $diskPaths, \IPS\ROOT_PATH . "/themes/frontpage/pages/" );
		
		$databasePaths = array();
		foreach( \IPS\Db::i()->select( '*', 'frontpage_pages', array( 'page_type=?', 'html' ) ) as $page )
		{
		...
		if ( \count( $diskPaths ) )
		{
			foreach( $diskPaths as $path )
			{
				...
				{
					@unlink( \IPS\ROOT_PATH . "/themes/frontpage/pages/" . $path );
				}
			}
		}
	}
	...
	public static function getCurrentDesignersModeFilePaths( &$paths, $path )
	{
		if ( is_dir( $path ) )
		{
			foreach ( new \DirectoryIterator( $path ) as $dir )
			{
				...
				{
					$paths[] = trim( str_replace( \IPS\ROOT_PATH . '/themes/frontpage/pages/', '', $dir->getRealPath() ), '/' );
				}
			}
		}
	}
	...
	{
		$path = \IPS\ROOT_PATH . '/themes/frontpage/pages';
		$seen = array();
		...
		\IPS\Db::i()->delete( 'frontpage_pages', array( 'page_type=? and ' . \IPS\Db::i()->in( 'page_id', $seen, TRUE ), 'html' ) );
	}
		...
		if ( is_dir( $path ) )
		{
			foreach ( new \DirectoryIterator( $path ) as $dir )
			{
				...
				else
				{
					...
					$fullPath = trim( str_replace( str_replace( '\\', '/', \IPS\ROOT_PATH ) . '/themes/frontpage/pages', '', str_replace( '\\', '/', $dir->getRealPath() ) ), '/' );
					...
					try
					{
						$page = \IPS\frontpage\Pages\Page::load( $fullPath, 'page_full_path' );
					...
					catch( \OutOfRangeException $e )
					{
						...
						$page = new \IPS\frontpage\Pages\Page;
						$page->content     = $contents;
						...
						\IPS\Lang::saveCustom( 'frontpage', "frontpage_page_" . $page->id, $page->title );
					}
	...
	public function item()
	{
		...
		return \IPS\frontpage\Pages\PageItem::constructFromData( $data );
	}
}