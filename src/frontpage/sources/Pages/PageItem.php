...
class _PageItem extends \IPS\Content\Item implements \IPS\Content\Searchable
{
	 ...
	 * @brief	Title
	 */
	public static $title = 'frontpage_page';
	...
	{
		if ( ( \IPS\Application::load('frontpage')->default OR \IPS\Settings::i()->frontpage_use_different_gateway ) AND $itemData['page_default'] AND !$itemData['page_folder_id'] )
		{
			/* Are we using the gateway file? */
			if ( \IPS\Settings::i()->frontpage_use_different_gateway )
			{
				/* Yes, work out the proper URL. */
				return \IPS\Http\Url::createFromString( \IPS\Settings::i()->frontpage_root_page_url, TRUE );
			}
			else
			{
				/* No - that's easy */
		...
		{
			return \IPS\Http\Url::internal( 'app=frontpage&module=pages&controller=page&path=' . $itemData['page_full_path'], 'front', 'content_page_path', array( $itemData['page_full_path'] ) );
		}
	}
	...
	{
		$indexData['index_title'] = \IPS\Member::loggedIn()->language()->addToStack( 'frontpage_page_' . $indexData['index_item_id'] );
	...
	public function searchIndexTitle()
	{
		...
		{
			$titles[] = $lang->get("frontpage_page_{$this->id}");
		}
	...
	public function searchIndexContent()
	{
		...
		{
			$content = array();
			foreach( \IPS\Db::i()->select( '*', 'frontpage_page_widget_areas', array( 'area_page_id=?', $this->id ) ) as $widgetArea )
			{
	...
	public function searchIndexPermissions()
	{
		try
		{
			return \IPS\Db::i()->select( 'perm_view', 'core_permission_index', array( "app='frontpage' AND perm_type='pages' AND perm_type_id=?", $this->id ) )->first();
		}
