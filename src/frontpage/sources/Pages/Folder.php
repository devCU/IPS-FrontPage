...
class _Folder extends \IPS\Node\Model
{
	 ...
	 */
	public static $subnodeClass = 'IPS\frontpage\Pages\Page';
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'folder_name', $this->id ? $this->name : '', TRUE, array( 'maxLength' => 64 ), function( $val )
		{
			try
			{
				$testPage = \IPS\frontpage\Pages\Page::load( $val, 'page_seo_name' );
			...
			try
			{
				$test = \IPS\frontpage\Pages\Folder::load( \IPS\Http\Url\Friendly::seoTitle( $val ), 'folder_name' );
			...
			catch ( \OutOfRangeException $e )
			{
				...
				if ( \IPS\Request::i()->folder_parent_id == 0 AND \IPS\frontpage\Pages\Page::isFurlCollision( \IPS\Http\Url\Friendly::seoTitle( $val ) ) )
				{
	...
	{
		$pages = (int) \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_pages', array( 'page_folder_id=?', $this->id ) )->first();
	...
	public function deleteOrMoveForm( $showMoveToChildren=FALSE )
	{
		...
		if ( $hasContent )
		{
			$form->add( new \IPS\Helpers\Form\Node( 'frontpage_move_pages', 0, TRUE, array( 'class' => \get_class( $this ), 'disabled' => array( $this->_id ), 'disabledLang' => 'node_move_delete', 'zeroVal' => 'frontpage_move_to_root', 'subnodes' => FALSE, 'permissionCheck' => function( $node )
			{
	...
	{
		if ( $values['frontpage_move_pages'] )
		{
			$folderId = $values['frontpage_move_pages']->_id;
		}
		else
		{
			$folderId = 0;
		...
		\IPS\Db::i()->update( 'frontpage_pages', array( 'page_folder_id' => $folderId ), array( 'page_folder_id=?', $this->_id ) );
		...
		/* Update pages */
		\IPS\frontpage\Pages\Page::resetPath( $folderId );
	...
	public function resetPath( $recursivelyCheck=true )
	{
		...
		/* Update pages */
		\IPS\frontpage\Pages\Page::resetPath( $this->id );
