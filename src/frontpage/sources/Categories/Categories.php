...
class _Categories extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	 ...
	 * @return	NULL|\IPS\frontpage\Categories object
	 */
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'category_furl_name', $this->furl_name, FALSE, array(), function( $val )
		{
			...
			if ( \IPS\Request::i()->category_parent_id == 0 and \IPS\frontpage\Categories::isFurlCollision( $val ) )
			{
			...
			try
			{
				$cat = \IPS\Db::i()->select( '*', 'frontpage_database_categories', array( 'category_database_id=? and category_parent_id=? and category_furl_name=?', \IPS\Request::i()->database_id, \IPS\Request::i()->category_parent_id, $val ) )->first();
			}
		...
		$form->add( new \IPS\Helpers\Form\Node( 'category_parent_id', ( ! $this->id ) ? ( isset( \IPS\Request::i()->parent ) ? \IPS\Request::i()->parent : 0 ) : $this->parent_id, FALSE, array(
			'class'		      => '\IPS\frontpage\Categories' . \IPS\Request::i()->database_id,
			'disabled'	      => false,
		...
		$form->addHeader('frontpage_categories_header_display');
		$templatesList     = array( 0 => \IPS\Member::loggedIn()->language()->addToStack('frontpage_categories_use_database') );
		$templatesDisplay  = array( 0 => \IPS\Member::loggedIn()->language()->addToStack('frontpage_categories_use_database') );
		...
		$options	= array();
		$fieldClass	= 'IPS\frontpage\Fields' . \IPS\Request::i()->database_id;
	...
	public function formatFormValues( $values )
	{
		...
		{
			\IPS\Lang::saveCustom( 'frontpage', 'content_cat_name_' . $this->id, $values['category_name'] );
		}

		if( isset( $values['category_description'] ) )
		{
			\IPS\Lang::saveCustom( 'frontpage', 'content_cat_name_' . $this->id . '_desc', $values['category_description'] );
		...
		if ( isset( $name ) AND empty( $values['category_furl_name'] ) )
		{
			...
			try
			{
				$cat = \IPS\frontpage\Categories::load( \IPS\Http\Url\Friendly::seoTitle( $name ), 'category_furl_name' );
			...
			if ( \IPS\Request::i()->category_parent_id == 0 and \IPS\frontpage\Categories::isFurlCollision( \IPS\Http\Url\Friendly::seoTitle( $name ) ) )
			{
				$ok = FALSE;
			}
	...
	public function cloneDatabasePermissions()
	{
		...
		$dbPerms  = \IPS\frontpage\Databases::load( $this->database_id )->permissions();
	...
	protected function get__items()
	{
		...
		{
			return \IPS\Db::i()->select('count(*)', 'frontpage_custom_database_' . $this->database_id, array("record_future_date=0 AND category_id=? AND record_approved=1 AND member_id=?", $this->id, \IPS\Member::loggedIn()->member_id) )->first();
		}
	...
	public function setLastComment( \IPS\Content\Comment $comment=NULL )
	{
		...
		{   
			try
			{
				$recordClass  = '\IPS\frontpage\Records' . $this->database_id;
				$commentClass = '\IPS\frontpage\Records\Comment' . $this->database_id;
				$comment      = NULL;
				...
				{
					if ( static::$latestRecordAdded->record_comments AND ( $database->_comment_bump & \IPS\frontpage\Databases::BUMP_ON_COMMENT ) )
					{
						...
						else
						{
							try
							{
								$comment = $commentClass::constructFromData( \IPS\Db::i()->select( '*', 'frontpage_database_comments', array( 'comment_record_id=? AND comment_approved=1', \IPS\Db::i()->select( 'primary_id_field', 'frontpage_custom_database_' . $this->database_id, array( 'category_id=? AND record_approved=1', $this->id ), 'record_last_comment DESC', 1 )->first() ), 'comment_date DESC', 1, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first() );
							}
		...
		if ( $comment !== NULL and ( $database->_comment_bump & \IPS\frontpage\Databases::BUMP_ON_COMMENT ) )
		{
		...
		$this->records        = \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_custom_database_' . $this->database_id, array( 'record_approved=1 AND record_future_date=0 AND category_id=?', $this->id ), NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
		$this->records_queued = \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_custom_database_' . $this->database_id, array( 'record_approved=0 AND category_id=?', $this->id ), NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
		$this->records_future = \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_custom_database_' . $this->database_id, array( 'record_future_date=1 AND category_id=?', $this->id ), NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
	...
	public function getLastCommentTime()
	{
		if( !$this->can_view_others and !\IPS\Member::loggedIn()->modPermission( 'can_content_view_others_records' ) )
		{
			try
			{
				$select = \IPS\Db::i()->select('record_last_comment', 'frontpage_custom_database_' . $this->database_id, array("record_future_date=0 AND category_id=? AND record_approved=1 AND member_id=?", $this->id, \IPS\Member::loggedIn()->member_id), 'record_last_comment DESC', 1 )->first();
			}
			catch ( \UnderflowException $e )
			{
				return NULL;
	...
	public function lastPost()
	{
		...
		else
		{
			try
			{
				$record = $RecordsClass::constructFromData( \IPS\Db::i()->select('*', 'frontpage_custom_database_' . $this->database_id, array("record_future_date=0 AND category_id=? AND record_approved=1 AND member_id=?", $this->id, \IPS\Member::loggedIn()->member_id), 'record_last_comment DESC', 1 )->first() );
	...
	protected function updateSearchIndexPermissions()
	{
		...
		else
		{
			...
			\IPS\Content\Search\Index::i()->massUpdate( 'IPS\frontpage\Records' . static::database()->_id, $this->_id, NULL, $permissions, NULL, NULL, NULL, NULL, NULL, TRUE );
			\IPS\Content\Search\Index::i()->massUpdate( 'IPS\frontpage\Records\Comment' . static::database()->_id, $this->_id, NULL, $permissions, NULL, NULL, NULL, NULL, NULL, TRUE );
		}
	}
	...
	{
		if ( isset( \IPS\Request::i()->cookie['frontpage_filters'] ) )
		{
			$saved = json_decode( \IPS\Request::i()->cookie['frontpage_filters'], TRUE );
	...
	{
		$cookie = ( isset( \IPS\Request::i()->cookie['frontpage_filters'] ) ) ? json_decode( \IPS\Request::i()->cookie['frontpage_filters'], TRUE ) : array();
		...
		\IPS\Request::i()->setCookie( 'frontpage_filters', json_encode( $cookie ), \IPS\DateTime::create()->add( new \DateInterval( 'P7D' ) ) );
	}
