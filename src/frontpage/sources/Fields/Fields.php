...
class _Fields extends \IPS\CustomField implements \IPS\Node\Permissions
{
	 ...
	 */
	public static $databaseTable = 'frontpage_database_fields';
	...
	*/
	 ...
	 */
	protected static $uploadStorageExtension = 'frontpage_Records';
	...
	public static function roots( $permissionCheck='view', $member=NULL, $where=array() )
	{
		...
		if ( ! isset( static::$cache[ static::$customDatabaseId ][ $permissionCheck ] ) )
		{
			...
			{
				if ( ! array_key_exists( $obj->type, static::$additionalFieldTypes ) AND ( ! class_exists( '\IPS\Helpers\Form\\' . mb_ucfirst( $obj->type ) ) AND ! class_exists( '\IPS\frontpage\Fields\\' . mb_ucfirst( $obj->type ) ) ) )
				{
	...
	{
		$key = 'frontpage_fieldids_' . static::$customDatabaseId;
		
		if ( ! isset( \IPS\Data\Store::i()->$key ) )
		{
			\IPS\Data\Store::i()->$key = iterator_to_array( \IPS\Db::i()->select( 'field_id', 'frontpage_database_fields', array( array( 'field_database_id=?', static::$customDatabaseId ) ) )->setKeyField('field_id') );
		}
	 ...
	 * @param	\IPS\Node\Model|NULL		$container			Parent container
	 * @param	int				            $flags				Bit flags
	 * @param   \IPS\frontpage\Records|NULL       $record             The record itself
	 * @return	array
	 */
	public static function fields( $values, $permissionCheck='view', \IPS\Node\Model $container=NULL, $flags=0, \IPS\frontpage\Records $record = NULL )
	{
		$fields        = array();
		$database      = \IPS\frontpage\Databases::load( static::$customDatabaseId );
	 ...
	 * @param	NULL|\IPS\frontpage\Record		$record				Record showing this field
	 ...
	 * @param	NULL|\IPS\frontpage\Records	$record	Record showing this field
	 * @note    Raw means the value saved from the input field, processed has the form display value method called. Listing and display take the options set by the field (badges, custom, etc)
	 *
	 * @return mixed|string
	...
	public function formatForDisplay( $value, $formValue, $type='listing', $record=NULL )
	{
		if ( $type === 'raw' )
		{
			...
			if ( $this->type === 'Item' )
			{
				...
				{
					$results = array();
					$class   = '\IPS\frontpage\Records' . $this->extra['database'];
		...
		else if ( $type === 'thumbs' and $this->type === 'Upload' )
		{
			if ( isset( $this->extra['thumbsize'] ) )
			{
				if ( $this->is_multiple )
				{
					$thumbs = iterator_to_array( \IPS\Db::i()->select( '*', 'frontpage_database_fields_thumbnails', array( array( 'thumb_field_id=? AND thumb_record_id=?', $this->id, $record->_id ) ) )->setKeyField('thumb_original_location')->setValueField('thumb_location') );
					$images = array();
				...
				else
				{
					try
					{
						return (string) \IPS\File::get( static::$uploadStorageExtension, \IPS\Db::i()->select( 'thumb_location', 'frontpage_database_fields_thumbnails', array( array( 'thumb_field_id=? AND thumb_record_id=?', $this->id, $record->_id ) ) )->first() )->url;
					}
					catch( \Exception $e ) { }
				}
			}
		...
		if ( isset( $options[ $type ]['method'] ) AND $options[ $type ]['method'] !== 'simple' )
		{
			if ( \in_array( $this->type, static::$mediaFields ) )
			{
				...
				{
					$class = '\IPS\frontpage\Fields\\' . $this->type;
					...
					else
					{
						try
						{
							$value = \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'global' )->$template( $formValue, $this->extra );
						}
						catch( \Exception $ex )
						{
							$value = $formValue;
		...
		else
		{
			...
			$value = \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'global' )->fieldDefault( $this->_title, $value );
		}
	...
	{
		$fixedFields = \IPS\frontpage\Databases::load( static::$customDatabaseId )->fixed_field_perms;
	...
	public static function setFixedFieldPermissions( $field, $values )
	{
		...
		\IPS\frontpage\Databases::load( static::$customDatabaseId )->fixed_field_perms = $fixedFields;
		\IPS\frontpage\Databases::load( static::$customDatabaseId )->save();
	}
	 ...
	 * @param	\IPS\frontpage\Fields	$field	The field
	 * @param	\IPS\frontpage\Records	$record	The record (if any)
	 * @return	void
	...
	public static function validateUnique( $val, $field, $record )
	{
		...
		if ( \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_custom_database_' . $database->id, $where )->first() )
		{
	...
	protected function get__badge()
	{
		...
		if ( \IPS\frontpage\Databases::load( $this->database_id )->field_title == $this->id )
		{
			$badge = array( 0 => 'positive ipsPos_right', 1 => 'content_fields_is_title' );
		}
		else if ( \IPS\frontpage\Databases::load( $this->database_id )->field_content == $this->id )
		{
	...
	protected function get__icon()
	{
		...
		else if ( class_exists( '\IPS\frontpage\Fields\\' . mb_ucfirst( $this->type ) ) )
		{
			return NULL;
		}
	...
	public function displayValue( $value=NULL, $showSensitiveInformation=FALSE )
	{
		...
		{
			/* Is special! */
			$class = '\IPS\frontpage\Fields\\' . mb_ucfirst( $this->type );
		...
		switch( mb_ucfirst( $this->type ) )
		{
			case 'Upload':
				...
				return \IPS\File::get( 'frontpage_Records', $value )->url;
			break;
			case 'Text':
			case 'TextArea':
			...
			case 'Item':
				...
				if ( \count( $value ) and isset( $this->extra['database'] ) and $this->extra['database'] )
				{
					...
					return \IPS\Theme::i()->getTemplate( 'global', 'frontpage', 'front' )->basicRelationship( $results );
				}
	...
	public function form( &$form )
	{
		...
		if ( $this->type )
		{
			...
			if ( !$ok )
			{
				\IPS\Output::i()->output .= \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( \IPS\Member::loggedIn()->language()->addToStack( 'frontpage_field_no_type_warning', NULL, array( 'sprintf' => array( $this->type ) ) ), 'warning', NULL, FALSE );
			}
		}
		...
		if( !$isTitleField )
		{
			...
			foreach( \IPS\frontpage\Databases::databases() as $db )
			{
				...
				{
					$disabled[] = $db->id;
					$databases[ $db->id ] = \IPS\Member::loggedIn()->language()->addToStack( 'frontpage_db_relational_with_name_disabled', NULL, array( 'sprintf' => array( $db->_title ) ) );
				}
			}
			if ( ! \count( $databases ) )
			{
				$databases[0] = \IPS\Member::loggedIn()->language()->addToStack('frontpage_relational_field_no_dbs');
				$disabled[] = 0;
			}
		...
		$form->add( new \IPS\Helpers\Form\Radio( 'field_upload_is_image', $this->id ? ( ( isset( $this->extra['type'] ) and $this->extra['type'] == 'image' ) ? 'yes' : 'no' ) : 'yes', TRUE, array(
			'options'	=> array(
				'yes' => 'frontpage_upload_field_is_image',
				'no'  => 'frontpage_upload_field_is_not_image',
		...
		{
			$usingForum = \IPS\frontpage\Databases::load( \IPS\Request::i()->database_id )->forum_record;
			
			if ( ! $usingForum and $this->id )
			{
				$usingForum = \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_database_categories', array( 'category_database_id=? and category_forum_override=1 and category_forum_record=1', $this->id ) );
			}
		...
		if ( $this->id AND ( $this->id == \IPS\frontpage\Databases::load( static::$customDatabaseId )->field_title OR $this->id == \IPS\frontpage\Databases::load( static::$customDatabaseId )->field_content ) )
		{
			$isTitleOrContent = TRUE;

			if ( $this->id == \IPS\frontpage\Databases::load( static::$customDatabaseId )->field_title )
			{
				$form->addMessage( 'field_display_opts_title', 'ipsMessage ipsMessage_info' );
			}

			if ( $this->id == \IPS\frontpage\Databases::load( static::$customDatabaseId )->field_content )
			{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'field_key', $this->id ? $this->key : FALSE, FALSE, array(), function( $val )
		{
			try
			{
				...
				throw new \InvalidArgumentException('frontpage_field_key_not_unique');
			}
		...
		{
			$displayOptions[ $id ] = $listingOptions[ $id ] = \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'global' )->fieldBadge( \IPS\Member::loggedIn()->language()->addToStack('frontpage_badge_label'), \IPS\Member::loggedIn()->language()->addToStack('frontpage_badge_value'), 'ipsBadge_style' . $id );
			$listingToggles[ $id ] = array( 'field_display_listing_json_badge_right' );
		}

		$displayOptions['simple'] = $listingOptions['simple'] = \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'global' )->fieldDefault( \IPS\Member::loggedIn()->language()->addToStack('frontpage_badge_label'), \IPS\Member::loggedIn()->language()->addToStack('frontpage_badge_value' ) );
		...
		$form->add( new \IPS\Helpers\Form\Codemirror( 'field_display_listing_json_custom', ( isset( $displayJson['listing']['html'] ) ? $displayJson['listing']['html'] : NULL ), FALSE, array( 'placeholder' => '{label}: {value}' ), function( $val )
        {
            ...
            {
	            throw new \LogicException('frontpage_field_error_bad_syntax');
            }
		...
		/* Display where? */
		$form->add( new \IPS\Helpers\Form\Radio( 'field_display_display_json_where', ( isset( $displayJson['display']['where'] ) ? $displayJson['display']['where'] : 'top' ), FALSE, array( 'options' => array( 'top' => 'frontpage_field_display_top', 'bottom' => 'frontpage_field_display_bottom' ) ), NULL, NULL, NULL, 'field_display_display_json_where' ) );
		...
		\IPS\Output::i()->globalControllers[]  = 'frontpage.admin.fields.form';
		\IPS\Output::i()->jsFiles  = array_merge( \IPS\Output::i()->jsFiles, \IPS\Output::i()->js( 'admin_fields.js', 'frontpage' ) );

		\IPS\Output::i()->title  = ( $this->id ) ? \IPS\Member::loggedIn()->language()->addToStack('frontpage_edit_field', FALSE, array( 'sprintf' => array( $this->_title ) ) ) : \IPS\Member::loggedIn()->language()->addToStack('frontpage_add_field');
	}
	...
	{
		static::$contentDatabaseTable = 'frontpage_custom_database_' . static::$customDatabaseId;
		...
		{
			\IPS\Lang::saveCustom( 'frontpage', static::$langKey . '_' . $this->id, $values['field_title'] );
		}
		
		if ( isset( $values['field_description'] ) )
		{
			\IPS\Lang::saveCustom( 'frontpage', static::$langKey . '_' . $this->id . '_desc', $values['field_description'] );
			unset( $values['field_description'] );
		}
		
		if ( array_key_exists( 'field_validator_error', $values ) )
		{
			\IPS\Lang::saveCustom( 'frontpage', static::$langKey . '_' . $this->id . '_validation_error', $values['field_validator_error'] );
		...
		if ( isset( $values['field_type'] ) AND $values['field_type'] === 'Item' )
		{
			...
			{
				throw new \LogicException( \IPS\Member::loggedIn()->language()->addToStack('frontpage_relational_field_no_db_selected') );
			}
	...
	public function delete( $skipDrop=FALSE )
	{
		...
		\IPS\Db::i()->delete( 'frontpage_database_fields_reciprocal_map', array( 'map_origin_database_id=? AND map_field_id=? ', static::$customDatabaseId, $this->id ) );
	...
	public function buildHelper( $value=NULL, $customValidationCode=NULL, \IPS\frontpage\Records $record = NULL, $flags=0 )
	{
		...
		switch ( mb_ucfirst( $this->type ) )
		{
			...
			case 'Item':
				...
				$options['class']    = '\IPS\frontpage\Records' . $this->extra['database'];
				break;
			case 'Number':
		...
		if ( $this->validator AND $this->validator_custom )
		{
			switch( mb_ucfirst( $this->type ) )
			{
				...
				case 'TextArea':
					...
					{
						$customValidationCode = 'IPS\frontpage\Fields' . static::$customDatabaseId . '::validateInput_' . $this->id;
					}
				break;
			}
		}
