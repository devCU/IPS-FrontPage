 ...
 *
 * @brief		FrontPage Database Reviews API
...
class _reviews extends \IPS\Content\Api\CommentController
{
	...
	/**
	 * GET /frontpage/reviews/{database_id}
	...
	public function GETindex( $database )
	{
		...
		try
		{
			...
			$this->class = 'IPS\frontpage\Records\Review' . $database->id;
		}
	...
	/**
	 * GET /frontpage/reviews/{database_id}/{id}
	 ...
	 * @return		\IPS\frontpage\Records\Review
	 */
	public function GETitem( $database, $review )
	{
		...
		try
		{
			$class = 'IPS\frontpage\Records\Review' . $database->id;
			if ( $this->member )
			{
	...
	/**
	 * POST /frontpage/reviews/{database_id}
	 * Create a comment
	 *
	...
	/**
	 * POST /frontpage/reviews/{database_id}/{review_id}
	 * Edit a comment
	 *
	 * @note		For requests using an OAuth Access Token for a particular member, any parameters the user doesn't have permission to use are ignored (for example, hidden will only be honoured if the authenticated user has permission to hide content).
	 * @param		int			$database		Database ID
	...
	/**
	 * DELETE /frontpage/reviews/{database_id}/{review_id}
	 * Deletes a comment
	 *
	 * @param		int			$database			Database ID
	 * @param		int			$review				Comment ID
