...
class _Records
{
	...
	public function attachmentPermissionCheck( $member, $id1, $id2, $id3, $attachment, $viewOnly=FALSE )
	{
		...
		try
		{
			$recordClass = '\IPS\frontpage\Records' . $id3;
	...
	public function attachmentLookup( $id1, $id2, $id3 )
	{
		try
		{
			if ( $id3 )
			{
				...
				$return->url(); // Need to check that won't throw an exception later, which might happen if the database no longer has a fpage
				return $return;
			}
			else
			{
