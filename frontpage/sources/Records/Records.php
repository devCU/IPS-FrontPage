	...
	\IPS\Content\MetaData
{
	...
	{
		return (bool) \IPS\frontpage\Databases::load( static::$customDatabaseId )->search;
	}
	...
	public static function constructFromData( $data, $updateMultitonStoreIfExists = TRUE )
	{
		...
		{
			$obj::$commentClass = 'IPS\frontpage\Records\CommentTopicSync' . static::$customDatabaseId;
		}
	...
	public static function getCommentsPerFpage()
	{
		if ( ! empty( \IPS\frontpage\Databases\Dispatcher::i()->recordId ) )
		{
			...
			try
			{
				$record = $class::load( \IPS\frontpage\Databases\Dispatcher::i()->recordId );
	...
	public static function database()
	{
		...
		{
			static::$database[ static::$customDatabaseId ] = \IPS\frontpage\Databases::load( static::$customDatabaseId );
		}
	...
	public static function loadFromSlugHistory( $slug )
	{
		...
		try
		{
			$row = \IPS\Db::i()->select( '*', 'frontpage_url_store', array( 'store_type=? and store_path=?', 'record', $slug ) )->first();
	...
	public function processForm( $values )
	{
		...
		{
			$revision = new \IPS\frontpage\Records\Revisions;
		...
		{			
			...
			{			
				if ( $this->record_image )
				{
					try
					{
						\IPS\File::get( 'frontpage_Records', $this->record_image )->delete();
					}
					catch ( \Exception $e ) { }
				}
				if ( $this->record_image_thumb )
				{
					try
					{
						\IPS\File::get( 'frontpage_Records', $this->record_image_thumb )->delete();
					}
					catch ( \Exception $e ) { }
				}
			...
			else
			{
				...
				if ( isset( $fixedFieldSettings['record_image']['thumb_dims'] ) )
				{
					...
					$thumb = $values['record_image']->thumbnail( 'frontpage_Records', $fixedFieldSettings['record_image']['thumb_dims'][0], $fixedFieldSettings['record_image']['thumb_dims'][1] );
				}
				else
				{
		...
		/* Other data */
		if ( $this->_new OR $database->_comment_bump & \IPS\frontpage\Databases::BUMP_ON_EDIT )
		{
		...
		foreach( $fieldObjects as $id => $row )
		{
			...
			if ( $row->can( ( $isNew ? 'add' : 'edit' ) ) and $row->type == 'Upload' )
			{
				...
				if ( $row->extra['type'] == 'image' and isset( $row->extra['thumbsize'] ) )
				{
					...
					$field = 'field_' . $row->id;
					$extra = $row->extra;
					$thumbs = iterator_to_array( \IPS\Db::i()->select( '*', 'frontpage_database_fields_thumbnails', array( array( 'thumb_field_id=?', $row->id ) ) )->setKeyField('thumb_original_location')->setValueField('thumb_location') );
					...
					if ( $this->$field  )
					{
						foreach( explode( ',', $this->$field ) as $img )
						{
							try
							{
								$original = \IPS\File::get( 'frontpage_Records', $img );
								
								try
								{								
									$thumb = $original->thumbnail( 'frontpage_Records', $dims[0], $dims[1] );
									
									if ( isset( $thumbs[ (string) $original ] ) )
									{
										\IPS\Db::i()->delete( 'frontpage_database_fields_thumbnails', array( array( 'thumb_original_location=? and thumb_field_id=? and thumb_record_id=?', (string) $original, $row->id, $this->primary_id_field ) ) );
										
										try
										{
											\IPS\File::get( 'frontpage_Records', $thumbs[ (string) $original ] )->delete();
										}
										catch ( \Exception $e ) { }
									}
									
									\IPS\Db::i()->insert( 'frontpage_database_fields_thumbnails', array(
						...
						$orphans = iterator_to_array( \IPS\Db::i()->select( '*', 'frontpage_database_fields_thumbnails', array( array( 'thumb_record_id=?', $this->primary_id_field ), array( 'thumb_field_id=?', $row->id ), array( \IPS\Db::i()->in( 'thumb_original_location', explode( ',', $this->$field ), TRUE ) ) ) ) );
						...
						if ( \count( $orphans ) )
						{
							foreach( $orphans as $thumb )
							{
								try
								{
									\IPS\File::get( 'frontpage_Records', $thumb['thumb_location'] )->delete();
								}
								catch ( \Exception $e ) { }
							}
							
							\IPS\Db::i()->delete( 'frontpage_database_fields_thumbnails', array( array( 'thumb_record_id=?', $this->primary_id_field ), array( 'thumb_field_id=?', $row->id ), array( \IPS\Db::i()->in( 'thumb_original_location', explode( ',', $this->$field ), TRUE ) ) ) );
						}
					}
				}
			}
			...
			{
				\IPS\Db::i()->delete( 'frontpage_database_fields_reciprocal_map', array( 'map_origin_database_id=? and map_field_id=? and map_origin_item_id=?', static::$customDatabaseId, $row->id, $this->_id ) );
	...
	public static function searchResultExtraData( $items )
	{
		...
		{
			$categoryPaths = iterator_to_array( \IPS\Db::i()->select( array( 'category_id', 'category_full_path' ), 'frontpage_database_categories', \IPS\Db::i()->in( 'category_id', $categoryIds ) )->setKeyField('category_id')->setValueField('category_full_path') );
	...
	public static function searchResultSnippet( array $indexData, array $authorData, array $itemData, array $containerData = NULL, array $reputationData, $reviewRating, $view )
	{
		...
		if ( $view == 'expanded' OR ( $view == 'condensed' AND isset( $itemData['record_image'] ) ) )
		{
			...
			return \IPS\Theme::i()->getTemplate( 'global', 'frontpage', 'front' )->recordResultSnippet( $indexData, $itemData, $url, $view == 'condensed' );
		}
		else
		{
			return NULL;
		...
		foreach( $customFields as $key => $field )
		{
			...
			if ( \get_class( $field ) == 'IPS\Helpers\Form\Upload' )
			{
				if ( \is_array( $fieldValues[ $fieldName ] ) )
				{
					...
					{
						$obj = \IPS\File::get( 'frontpage_Records', $fileName );
						if ( $obj->isImage() )
						{
							$attachments[] = array( 'frontpage_Records' => $fileName );
						}
					}
				}
				else
				{
					$obj = \IPS\File::get( 'frontpage_Records', $fieldValues[ $fieldName ] );
					if ( $obj->isImage() )
					{
						$attachments[] = array( 'frontpage_Records' => $fieldValues[ $fieldName ] );
					}
				}
			}
		}
	...
	public function get__title()
	{
		...
		try
		{
			...
			{
				$class = '\IPS\frontpage\Fields' .  static::database()->id;
	...
	public function editLine()
	{
		...
		{
			return \IPS\frontpage\Theme::i()->getTemplate( static::database()->template_display, 'frontpage', 'database' )->recordEditLine( $this );
		}
		return NULL;
	}
	...
	public function save()
	{
		...
		if ( $new OR static::database()->_comment_bump & \IPS\frontpage\Databases::BUMP_ON_EDIT )
		{
	...
	public function getReciprocalItems()
	{
		...
		if ( \IPS\frontpage\Databases::hasReciprocalLinking( static::database()->_id ) )
		{
			$return = array();
			/* Oh that's just lovely then. Lets be a good fellow and fetch the items then! */
			foreach( \IPS\Db::i()->select( '*', 'frontpage_database_fields_reciprocal_map', array( 'map_foreign_database_id=? and map_foreign_item_id=?', static::database()->_id, $this->primary_id_field ) ) as $record )
			{
				try
				{
					$recordClass = 'IPS\frontpage\Records' . $record['map_origin_database_id'];
	...
	public function comments( $limit=NULL, $offset=NULL, $order='date', $orderDirection='asc', $member=NULL, $includeHiddenComments=NULL, $cutoff=NULL, $extraWhereClause=NULL, $bypassCache=FALSE, $includeDeleted=FALSE, $canViewWarn=NULL )
	{
		if ( $this->useForumComments() )
		{
			$recordClass = 'IPS\frontpage\Records\RecordsTopicSync' . static::$customDatabaseId;
	...
	public function commentReviewTabs()
	{
		...
		{
			$tabs['reviews'] = \IPS\Member::loggedIn()->language()->addToStack( 'frontpage_review_count', TRUE, array( 'pluralize' => array( $this->mapped('num_reviews') ) ) );
		}
		if ( static::database()->options['comments'] )
		{
			...
			$tabs['comments'] = \IPS\Member::loggedIn()->language()->addToStack( 'frontpage_comment_count', TRUE, array( 'pluralize' => array( $count ) ) );
		}
	...
	public function commentReviews( $tab )
	{
		if ( $tab === 'reviews' )
		{
			return \IPS\frontpage\Theme::i()->getTemplate( static::database()->template_display, 'frontpage', 'database' )->reviews( $this );
		}
		elseif( $tab === 'comments' )
		{
			return \IPS\frontpage\Theme::i()->getTemplate( static::database()->template_display, 'frontpage', 'database' )->comments( $this );
		}
	...
	public function processCommentForm( $values )
	{
		if ( $this->useForumComments() )
		{
			...
			$commentClass = 'IPS\frontpage\Records\CommentTopicSync' . static::$customDatabaseId;
	...
	{
		if ( ( static::database()->_comment_bump & ( \IPS\frontpage\Databases::BUMP_ON_EDIT + \IPS\frontpage\Databases::BUMP_ON_COMMENT ) and $this->record_edit_time > 0 and $this->record_edit_time > $this->record_last_comment ) OR
			 ( ( static::database()->_comment_bump & \IPS\frontpage\Databases::BUMP_ON_EDIT ) and !( static::database()->_comment_bump & ( \IPS\frontpage\Databases::BUMP_ON_EDIT + \IPS\frontpage\Databases::BUMP_ON_COMMENT ) ) and $this->record_edit_time > 0 ) )
		{
			try
			{
     ...
     *
     * @param   \IPS\forums\Topic   $topic  Forums topic
	 * @return  \IPS\frontpage\Records|NULL
	 */
	...
	public function syncTopic( $commentCheck = TRUE )
	{
		...
		catch ( \LogicException $e )
		{
			...
			\IPS\Log::log( sprintf( "Record %s in database %s tried to sync the topic, but failed because it has no valid url", $this->$idColumn , static::$customDatabaseId), 'frontpage_topicsync' );
			return;
		}
		...
		if ( $this->record_topicid )
		{
			...
			try
			{
				...
				$content = \IPS\Theme::i()->getTemplate( 'submit', 'frontpage', 'front' )->topic( $this );
	...
	public function delete()
	{
		...
		\IPS\Db::i()->delete( 'frontpage_database_fields_reciprocal_map', array( 'map_origin_database_id=? and map_origin_item_id=?', static::database()->id, $this->_id ) );
	...
	public function canEdit( $member=NULL )
	{
		...
		if ( parent::canEdit( $member ) )
		{
			...
			return $this->container()->can( 'edit', $member ) or $this->container()->modPermission( 'edit', $member, \get_called_class() );
		}
	 ...
	 * @return  \IPS\frontpage\Records\Comment|bool
	 */
	public function addCommentWhenFiltersChanged( $values )
	{
		...
		$commentClass  = 'IPS\frontpage\Records\Comment' . static::$customDatabaseId;
		$categoryClass = 'IPS\frontpage\Categories' . static::$customDatabaseId;
		$fieldsClass   = 'IPS\frontpage\Fields' . static::$customDatabaseId;
		$newValues     = array();
		...
		if ( \count( $diff ) )
		{
			...
			foreach( $diff as $name => $value )
			{
				...
				if ( $display[ $id ] )
				{
					$show[ $name ] = sprintf( \IPS\Member::loggedIn()->language()->get( 'frontpage_record_field_changed' ), \IPS\Member::loggedIn()->language()->get( 'content_field_' . $id ), $display[ $id ] );
				}
			}

			if ( \count( $show ) )
			{
				$post = \IPS\frontpage\Theme::i()->getTemplate( static::database()->template_display, 'frontpage', 'database' )->filtersAddComment( $show );
	 ...
	 * @apiresponse	\IPS\frontpage\Categories		category		Category
	...
	public static function getItemsWithPermissionWhere( $where, $permissionKey, $member, &$joinContainer, $skipPermission=FALSE )
	{
		...
		if ( $skipPermission !== TRUE and \in_array( $permissionKey, array( 'view', 'read' ) ) )
		{
			...
			{
				if ( $skipPermission instanceof \IPS\frontpage\Categories )
				{
					if ( !$skipPermission->can_view_others )
					{
						$where['item'][] = array( 'frontpage_custom_database_' . static::database()->id . '.member_id=?', $member->member_id );
					}
				}
				else
				{
					...
					$where[] = array( '( category_can_view_others=1 OR frontpage_custom_database_' . static::database()->id . '.member_id=? )', $member->member_id );
				}
			}
		}
	...
	public function embedContent( $params )
	{
		...
		return \IPS\Theme::i()->getTemplate( 'global', 'frontpage' )->embedRecord( $this, $this->url()->setQueryString( $params ) );
	}
	...
	public static function databaseTableCount()
	{
		...
		{
			return parent::databaseTableCount();
		}
	}
