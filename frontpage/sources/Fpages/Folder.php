...
class _Folder extends \IPS\Node\Model
{
	 ...
	 */
	public static $databaseTable = 'frontpage_folders';
	 ...
	 */
	public static $subnodeClass = 'IPS\frontpage\Fpages\Fpage';
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'folder_name', $this->id ? $this->name : '', TRUE, array( 'maxLength' => 64 ), function( $val )
		{
			try
			{
				$testFpage = \IPS\frontpage\Fpages\Fpage::load( $val, 'fpage_seo_name' );
				
				/* Ok, we have a folder, but is it on the same tree as us ?*/
				if ( \intval( \IPS\Request::i()->folder_parent_id ) == $testFpage->folder_id )
				{
			...
			try
			{
				$test = \IPS\frontpage\Fpages\Folder::load( \IPS\Http\Url\Friendly::seoTitle( $val ), 'folder_name' );
			...
			catch ( \OutOfRangeException $e )
			{
				...
				if ( \IPS\Request::i()->folder_parent_id == 0 AND \IPS\frontpage\Fpages\Fpage::isFurlCollision( \IPS\Http\Url\Friendly::seoTitle( $val ) ) )
				{
	...
	{
		$fpages = (int) \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_fpages', array( 'fpage_folder_id=?', $this->id ) )->first();
		$subFolders = (int) \IPS\Db::i()->select( 'COUNT(*)', 'frontpage_folders', array( 'folder_parent_id=?', $this->id ) )->first();

		return $fpages + $subFolders;
	}
	...
	public function deleteOrMoveForm( $showMoveToChildren=FALSE )
	{
		...
		if ( $hasContent )
		{
			$form->add( new \IPS\Helpers\Form\Node( 'frontpage_move_fpages', 0, TRUE, array( 'class' => \get_class( $this ), 'disabled' => array( $this->_id ), 'disabledLang' => 'node_move_delete', 'zeroVal' => 'frontpage_move_to_root', 'subnodes' => FALSE, 'permissionCheck' => function( $node )
			{
	...
	{
		if ( $values['frontpage_move_fpages'] )
		{
			$folderId = $values['frontpage_move_fpages']->_id;
		}
		else
		{
			$folderId = 0;
		...
		\IPS\Db::i()->update( 'frontpage_fpages', array( 'fpage_folder_id' => $folderId ), array( 'fpage_folder_id=?', $this->_id ) );
		\IPS\Db::i()->update( 'frontpage_folders', array( 'folder_parent_id' => $folderId ), array( 'folder_id=?', $this->_id ) );
		\IPS\Db::i()->update( 'frontpage_folders', array( 'folder_parent_id' => $folderId ), array( 'folder_parent_id=?', $this->_id ) );
		
		unset( \IPS\Data\Store::i()->fpages_fpage_urls );
		
		/* Update fpages */
		\IPS\frontpage\Fpages\Fpage::resetPath( $folderId );
	...
	public function resetPath( $recursivelyCheck=true )
	{
		...
		/* Update fpages */
		\IPS\frontpage\Fpages\Fpage::resetPath( $this->id );
