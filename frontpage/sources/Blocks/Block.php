...
namespace IPS\frontpage\Blocks;
...
class _Block extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	 ...
	 */
	public static $databaseTable = 'frontpage_blocks';
	 ...
	 */
	public static $parentNodeClass = 'IPS\frontpage\Blocks\Container';
	 ...
	 */
	public static $permissionLangPrefix = 'perm_frontpage_block_';
	 ...
	 *
	 * @param	string|int|\IPS\frontpage\Blocks\Block	$block	Block ID
	...
	public static function display( $block, $orientation=NULL )
	{
		try
		{
			try
			{
				...
				else if ( ! $block instanceof \IPS\frontpage\Blocks\Block )
				{
		...
		if ( ! \in_array( $functionName, array_keys( static::$calledTemplates ) ) )
		{
			if ( ! isset( \IPS\Data\Store::i()->$functionName ) )
			{
				...
				else if ( $this->template )
				{
					try
					{
						$template	= \IPS\frontpage\Templates::load( $this->template );
						$object		= \IPS\frontpage\Theme::i()->getTemplate( $template->group, 'frontpage', $template->location );
...
EOF;
	...
	public static function deleteCompiled( $ids=NULL )
	{
		if ( $ids === NULL )
		{
			$ids = iterator_to_array( \IPS\Db::i()->select( 'block_id', 'frontpage_blocks' )->setValueField('block_id') );
		}
	...
	public function form( &$form )
	{
		...
		$nodeContainer = $this->id ? $this->category :
			( \IPS\Request::i()->parent ?: \IPS\frontpage\Blocks\Container::load( ( $block_type == 'custom' ? 'block_custom' : 'block_plugins' ), 'container_key' )->id );
		$form->add( new \IPS\Helpers\Form\Node( 'block_category', $nodeContainer, TRUE, array(
				'class'    => '\IPS\frontpage\Blocks\Container',
				'subnodes' => false
		) ) );

		$form->addHeader( 'frontpage_block_form_display' );
		...
		$form->add( new \IPS\Helpers\Form\Text( 'block_key', $this->id ? $this->key : FALSE, FALSE, array(), function( $val )
		{
			try
			{
				...
				try
				{
					$block = \IPS\frontpage\Blocks\Block::load( $val, 'block_key');
				}
				catch( \OutOfRangeException $ex )
				{
					/* Doesn't exist? Good! */
				...
				throw new \InvalidArgumentException('frontpage_block_key_not_unique');
			}
		...
		if ( $block_type === 'plugin' and $block_plugin )
		{
			...
			if ( mb_substr( $block_plugin, 0, 8 ) === 'db_feed_' )
			{
				...
				$this->plugin_config = array( 'frontpage_rf_database' => $databaseId );

				/* JS needs this to produce the preview */
				$form->hiddenValues['frontpage_rf_database'] = $databaseId;
			}
			else if ( $this->id and $block_plugin === 'RecordFeed' )
			{
				$form->hiddenValues['frontpage_rf_database'] = $this->_plugin_config['frontpage_rf_database'];
			}
			...
			try
			{
				...
				\IPS\Output::i()->output .= \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( \IPS\Member::loggedIn()->language()->addToStack( 'frontpage_block_feed_form_message', FALSE, array( 'sprintf' => $this->widget()->title() ) ), 'information', NULL, FALSE );
			}
			catch ( \OutOfRangeException $ex )
			{
				throw new \LogicException( 'frontpage_error_block_plugin_not_found' );
			}
		...
		{
			$form->addHtml( \IPS\Theme::i()->getTemplate( 'blocks', 'frontpage', 'admin' )->previewBlock() );

			$templates = array( '_default_' => \IPS\Member::loggedIn()->language()->addToStack('content_block_template_use_default') );

			foreach( \IPS\frontpage\Templates::getTemplates( \IPS\frontpage\Templates::RETURN_BLOCK ) as $id => $obj )
			{
			...
			$form->add( new \IPS\Helpers\Form\Select( 'block_template_id', ( $this->id and $this->template ) ? $this->template : NULL, FALSE, array(
					'options' => $templates
			), NULL, NULL, \IPS\Theme::i()->getTemplate( 'blocks', 'frontpage', 'admin' )->previewTemplateLink( $block_plugin ), 'block_template_id' ) );
		...
		\IPS\Output::i()->cssFiles = array_merge( \IPS\Output::i()->cssFiles, \IPS\Theme::i()->css( 'templates/view.css', 'frontpage', 'admin' ) );
		\IPS\Output::i()->cssFiles = array_merge( \IPS\Output::i()->cssFiles, \IPS\Theme::i()->css( 'blocks/form.css', 'frontpage', 'admin' ) );

		\IPS\Output::i()->globalControllers[]  = 'frontpage.admin.blocks.form';
		\IPS\Output::i()->jsFiles  = array_merge( \IPS\Output::i()->jsFiles, \IPS\Output::i()->js( 'admin_blocks.js', 'frontpage' ) );
	...
	public function formatFormValues( $values )
	{
		...
		{
			\IPS\Lang::saveCustom( 'frontpage', "content_block_name_" . $this->id, $values['block_name'] );
		}

		if( isset( $values['block_description'] ) )
		{
			\IPS\Lang::saveCustom( 'frontpage', "content_block_name_" . $this->id . '_desc', $values['block_description'] );
			...
			else
			{
				...
				if ( isset( $values['block_save_as_template'] ) AND $values['block_save_as_template'] )
				{
					...
					else
					{
						try
						{
							$template = \IPS\frontpage\Templates::load( $values['block_template_id'] );
						...
						{
							throw new \LogicException('frontpage_error_no_template_found');
						}
					}
		...
		else if( isset( $values['block_content'] ) )
		{
			...
			if ( \IPS\Request::i()->block_editor === 'editor' )
			{
				\IPS\Lang::saveCustom( 'frontpage', "frontpage_block_content_{$this->id}", $values['block_content'] );
	...
	public function save()
	{
		...
		if ( $this->id )
		{
			...
			\IPS\frontpage\Widget::deleteCachesForBlocks( $this->key );
		}
