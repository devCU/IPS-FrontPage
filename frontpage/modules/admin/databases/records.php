...
{	
	...
	protected function manage()
	{
		...
		if ( !\IPS\Request::i()->database_id )
		{
			...
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&do=manage&database_id=' . $database->_id ) );
			}
		}
		...
		/* Create the table */
		$table = new \IPS\Helpers\Table\Db( 'frontpage_custom_database_' . \IPS\Request::i()->database_id, \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records' ) );
		...
		$table->advancedSearch = array(
				'category_id'	=> array( \IPS\Helpers\Table\SEARCH_NODE, array(
						'class'		      => '\IPS\frontpage\Categories',
						'disabled'	      => false,
		...
		if ( $database->use_categories )
		{
			\IPS\Output::i()->sidebar['actions']['add'] = array(
				...
				'title'	=> 'add',
				'icon'	=> 'plus',
				'link'	=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&do=select&database_id=' . \IPS\Request::i()->database_id ),
		...
		else
		{
			\IPS\Output::i()->sidebar['actions']['add'] = array(
				...
				'title'	=> 'add',
				'icon'	=> 'plus',
				'link'	=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&do=form&database_id=' . \IPS\Request::i()->database_id )
			);
		}
		...
		$table->rowButtons = function( $row )
		{
			...
			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'frontpage', 'databases', 'records_edit' ) )
			{
				$return['edit'] = array(
					'title' => 'edit',
					'icon' => 'pencil',
					'link' => \IPS\Http\Url::internal('app=frontpage&module=databases&controller=records&database_id=' . \IPS\Request::i()->database_id . '&id=' . $row['primary_id_field'] . '&do=form'),
					'data' => array()
				);
			}
			...
			$return['move']	= array(
						'title'	=> 'move',
						'icon'	=> 'arrow-right',
						'link'	=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&database_id=' . \IPS\Request::i()->database_id  . '&do=select&move=' . $row['primary_id_field'] ),
						'data'	=> array( 'ipsDialog' => '', 'ipsDialog-title' => \IPS\Member::loggedIn()->language()->addToStack('database_select_category' ) )
			);

			if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'frontpage', 'databases', 'records_delete' ) )
			{
				$return['delete']	= array(
							'title'	=> 'delete',
							'icon'	=> 'times-circle',
							'link'	=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&database_id=' . \IPS\Request::i()->database_id . '&id=' . $row['primary_id_field'] . '&do=delete' ),
							'data'	=> array( 'delete' => '' )
				);
			}
			return $return;
		...
		$table->parsers = array(
			'field_' . $database->field_title => function( $val, $row ) use ($database)
			{
				...
				return \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'admin' )->title( $row, $val );
			},
			'category_id'	=> function( $val ) use ($database)
			{
				if ( $val )
				{
					try
					{
						...
						return \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'admin' )->category( $class::load( $val ) );
					}
		...
		if ( \IPS\Member::loggedIn()->hasAcpRestriction( 'frontpage', 'databases', 'databases_edit' ) )
		{
			\IPS\Output::i()->sidebar['actions']['databasemanage'] = array(
				'title'		=> 'frontpage_manage_database',
				'icon'		=> 'wrench',
				'link'		=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=databases&do=form&id=' . $database->_id ),
				'data'	    => NULL
			);
		}

		\IPS\Output::i()->sidebar['actions']['databasepermissions'] = array(
			'title'		=> 'frontpage_database_permissions',
			'icon'		=> 'lock',
			'link'		=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=databases&do=permissions&id=' . $database->_id ),
			'data'	    => NULL
		);
	...
	public function delete()
	{
		...
		$recordClass  = '\IPS\frontpage\Records' . \IPS\Request::i()->database_id;
	...
	protected function select()
	{
		...
		$catClass = 'IPS\frontpage\Categories' . \IPS\Request::i()->database_id;
		$recordClass = 'IPS\frontpage\Records' . \IPS\Request::i()->database_id;
		$category    = NULL;
		$record      = NULL;
		...
		$form->add( new \IPS\Helpers\Form\Node( 'category', $category, TRUE, array(
			'url'					=> \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&do=select&database_id=' . \IPS\Request::i()->database_id . ( ( $move ) ? '&move=' . $move : '' ) ),
			'class'					=> $catClass,
			'multiple'              => false,
			'permissionCheck'		=> function( $node )
			{
		...
		if ( $values = $form->values() )
		{
			if ( $move and $record )
			{
				...
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&database_id=' . \IPS\Request::i()->database_id ), 'completed' );
			}
			else
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=frontpage&module=databases&controller=records&do=form&database_id=' . \IPS\Request::i()->database_id . '&category_id=' . $values['category']->id ) );
			}
		}
	...
	public function form()
	{
		...
		$class = 'IPS\frontpage\Records' . \IPS\Request::i()->database_id;
		...
		$fieldsClass = 'IPS\frontpage\Fields' . \IPS\Request::i()->database_id;
		$catClass    = 'IPS\frontpage\Categories' . \IPS\Request::i()->database_id;
		...
		if ( $values = $form->values() )
		{
			...
			if ( $changeAuthor )
			{
				...
				{
					/* If the database isn't attached to a fpage, a call to url() when logging an author change throws this */
				}
			}
	...
	public function categoryTree()
	{
		...
		try
		{
			...
			\IPS\Output::i()->output = \IPS\Theme::i()->getTemplate( 'global', 'core' )->block( \IPS\Member::loggedIn()->language()->addToStack('content_tree_title'), \IPS\Theme::i()->getTemplate( 'records', 'frontpage', 'admin' )->categoryTree( $category, $parents ), FALSE );
